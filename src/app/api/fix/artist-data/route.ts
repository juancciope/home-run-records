...
import { VibrateAPI } from '@/lib/services/viberate-api'
  ...
  try {
    ...
    if (!profile || (!profile.viberate_uuid && !profile.viberate_artist_id)) {
      ...
      if (profile?.artist_name) {
        const searchResults = await VibrateAPI.searchArtists(profile.artist_name)
        if (searchResults.length > 0) {
          // Use the first result
          const artistUuid = searchResults[0].uuid
          const artistData = await VibrateAPI.getArtistProfile(artistUuid)
