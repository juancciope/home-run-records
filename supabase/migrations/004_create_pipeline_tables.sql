...
-- Admin access policies (conditional on users table existence)
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users' AND table_schema = 'public') THEN
    EXECUTE 'CREATE POLICY "Admins can view all production records" ON production_records
      FOR SELECT USING (
        EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can view all marketing records" ON marketing_records
      FOR SELECT USING (
        EXISTS (
          SELECT 1 FROM public.users
          WHERE id = auth.uid() AND role = ''admin''
    ...
    EXECUTE 'CREATE POLICY "Admins can view all fan engagement records" ON fan_engagement_records
      FOR SELECT USING (
        EXISTS (
          SELECT 1 FROM public.users
          WHERE id = auth.uid() AND role = ''admin''
    ...
    EXECUTE 'CREATE POLICY "Admins can view all conversion records" ON conversion_records
      FOR SELECT USING (
        EXISTS (
          SELECT 1 FROM public.users
          WHERE id = auth.uid() AND role = ''admin''
    ...
    EXECUTE 'CREATE POLICY "Admins can view all agent records" ON agent_records
      FOR SELECT USING (
        EXISTS (
          SELECT 1 FROM public.users
          WHERE id = auth.uid() AND role = ''admin''
